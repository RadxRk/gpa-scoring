...
| Layer | Dimension | What it measures | Needs extra input? |
|-------|-----------|------------------|:---:|
...
| Trust | **HD** Hallucination Detection | Are claims/numbers/citations grounded, not fabricated? | No (L1) · tool results (L2) |
| Action | **EE** Execution Efficiency | Minimal, non-redundant steps? | Execution trace |
| Plan | **PQ** Plan Quality | Was an effective plan designed? | Execution trace |
| Plan | **PA** Plan Adherence | Did it follow the stated plan? | Execution trace |

**Why GF + LC + HD first.** These three are evaluable from the response text alone — no
agent internals required — so they ship today in a single judge call. EE/PQ/PA require the
...
**Why HD is its own dimension.** GF asks "did it answer?" and LC asks "is it
self-consistent?" — a response can score well on both while quietly fabricating a statistic
or citing a study that doesn't exist. In a clinical-safety context a confidently-wrong
number is the most dangerous failure mode, so detecting fabrication earns a dedicated axis
rather than being folded into GF.
...
JSON object. **Why:** N dimensions for the price of one call — GF + LC + HD cost a single
round-trip. The prompt is generated by mapping over `dimensions`, so adding HD required no
new call and no per-dimension parsing code. The trade-off is a longer prompt and the risk
that the model omits a key; we accept omissions silently (no retry) to keep the path simple
and cheap.

### Decision 3a — HD as two levels: judge first, evidence when available

Hallucination Detection ships in two depths that build on each other:

- **Level 1 (always on when HD is enabled)** — the judge rates fabrication from the response
  text alone. Free (rides the existing call), but it is a *plausibility* judgement: it
  catches invented citations and absurd numbers, not a believable number that is simply
  wrong.
- **Level 2 (when the host passes `toolResults`)** — the judge is also handed the raw data
  the agent's tools returned and grades whether the response *matches* it.

**Why two levels instead of one.** The canonical failure is a decimal shift — a tool returns
`0.82%`, the response says `8.2%`. Both are believable clinical numbers, so Level 1 (and a
human skim) pass it; only a direct comparison against ground truth catches it. We didn't
make tool grounding mandatory because not every host can cheaply surface tool payloads, and
HD Level 1 still adds value with zero integration cost. So Level 2 is **opt-in via one
optional field** (`toolResults`) and degrades gracefully to Level 1 when absent. The cost is
that HD's strength depends on what the host chooses to pass, and the evidence is injected
into the same string-built prompt (see the SQL-interpolation trade-off below).

*Alternative rejected:* a separate second LLM call dedicated to grounding — doubles latency
and cost for a check that fits in the existing prompt.
...
| HD Level 2 opt-in (grounding only if `toolResults` passed) | ✅ Yes — zero-integration L1 vs evidence-based L2 | Document expected payloads; consider host-side capture helper |
...
| Batch re-score ignores `toolResults` (HD always L1) | ⚠️ Partial | Thread tool results per message |
| SQL string interpolation (now incl. `toolResults`) | ⚠️ Partial | Move to bind variables |
...
2. **HD Level 3.** The design deck frames HD as "three levels, each building on the
   previous," but only Levels 1–2 are specified so far. A future Level 3 would move beyond
   the LLM judge to deterministic verification — e.g. regex/format validation of identifiers
   (NCT/DOI/PubMed), numeric reconciliation against tool results, and citation existence
   checks — surfacing hard evidence rather than a model judgement. To be scoped.
3. **Carry `toolResults` through batch re-score** so HD Level 2 works for re-evaluation, not
   just live scoring.
4. **Configurable Tier-2 extraction.** Replace the hardcoded `PV_PATTERN` with a
   `keyFindingsPattern` config option (neutral default), restoring true domain-agnosticism.
5. **Judge robustness.** Validate the judge's JSON against requested dimensions; retry once
   on a missing/invalid key.
6. **Observability hook.** Optional callback on scoring/persistence failure so operators can
   alert on judge errors without breaking the silent-degradation contract.
7. **Bind-variable SQL** and a **batch concurrency cap**.
