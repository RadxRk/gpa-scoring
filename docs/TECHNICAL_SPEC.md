...
interface ScoreRequest {
  ...
  threadId?:  string;
  messageId?: string;
  toolResults?: ToolResult[];   // optional — enables HD Level 2 (grounded check)
}

interface ToolResult {
  name?:    string;     // tool/function name, e.g. "acctmodel"
  type?:    string;     // event type, e.g. "tool_result"
  content:  string;     // raw payload the tool returned (usually a JSON string)
}
```

`question` and `response` are mandatory; missing either yields `400`. `toolResults` is
optional and only consumed when the `HD` dimension is enabled (see [§5.2](#52-hd-grounded-check-level-2)).
...
dimensions at once, so GF + LC + HD cost one LLM call. Shape:
...
source of truth for both the prompt and the badge's full-name tooltip. The `keys` and
rubric blocks are generated by mapping over `cfg.dimensions`, so a new dimension (like HD)
is included in the prompt and parsed back automatically with no per-dimension code.
...
5. GF and LC are mirrored into legacy flat fields for backwards compatibility. HD has no
   legacy mirror (it is a new dimension).

### 5.2 HD grounded check (Level 2)

When `HD` is among the dimensions **and** the request includes a non-empty `toolResults`
array, `buildGroundingBlock` (`scorer.ts`) appends an evidence block between the rubrics
...
Without `toolResults` (or when HD is not requested) the block is empty and HD is scored at
**Level 1** — plausibility from the response text and the HD rubric alone. The grounded
prompt is what lets the judge catch a *plausible but wrong* number (the canonical example:
a response stating `8.2%` when the tool returned `0.82%`), which Level 1 cannot detect.
...
| `hd_score / hd_label / hd_reasoning` | `NUMBER / VARCHAR / TEXT` | Hallucination Detection (NULL unless HD enabled) |
...
1. **EE / PQ / PA cannot be scored as built.** `ScoreRequest` carries `toolResults` (for
   HD) but no execution *trace*, and `buildPrompt` consumes only `question`, `response`,
   and `toolResults`. Enabling these dimensions asks the judge to grade plan/execution
   quality with no trace data, so results are unreliable. **Fix path:** add
   `trace?: AgentTrace` to `ScoreRequest`, thread it through `scoreResponse` →
   `buildPrompt`, and inject the trace-dependent rubric blocks only when a trace is present.
   Until then, keep `dimensions: ["GF","LC","HD"]`.

2. **HD Level 1 is plausibility-only.** Without `toolResults`, HD judges fabrication from
   the response text alone — it misses a plausible-looking but factually wrong number.
   Level 2 (passing `toolResults`) fixes this, but its accuracy is bounded by the fidelity
   and completeness of the tool payloads supplied, and grounding can be diluted if a long
   response was summarised (Tier 3) before scoring.

3. **`PV_PATTERN` is domain-coupled.** The Tier-2 `[KEY FINDINGS]` extractor is a
...
4. **SQL via string interpolation.** `esc()` escapes only `\` and `'` before inlining
   the (model-bound) prompt, response, and `toolResults` into `CORTEX.COMPLETE(...)`.
   Correct for the current escaping, but bind variables / parameter binding would be more
   robust against future edge cases.

5. **No dimension-completeness validation.** A dimension the model omits is silently
   dropped; there is no retry or schema check on the judge's JSON.

6. **Silent degradation.** Errors hide the badge with no user-facing signal beyond
   `console.error`. Intentional, but reduces observability of judge failures.

7. **Uncapped batch concurrency.** `useBatchGpaScoring` issues one request per assistant
   message via `Promise.allSettled` with no concurrency limit. It also does not forward
   `toolResults`, so batch re-scores always evaluate HD at Level 1.

8. **Latency floor.** Each score costs one synchronous `CORTEX.COMPLETE` round-trip
...
| LLM calls per response | 1 (GF + LC + HD in one prompt); 2 for summarised tier |
